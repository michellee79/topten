...
function save(){
    if (validate()){
        ...
        $.ajax({
            url : '/aajax/cms/saveemail/' + eId,
            type : 'post',
            dataType : 'json',
            contentType: false,
            processData: false,
            success: function(data){
                window.location = '/admin_cms_email';
            },
            data : fdata
        });
    } else{
...
function upload(){
    ...
    xhr.onload = function() {
        ...
        top.$('.mce-btn.mce-open').parent().find('.mce-textbox').val(json.location).closest('.mce-window').find('.mce-primary').click();
...
function returnToMenu(){
    location.replace('/admin_cms_email');
}
